...
@Slf4j
public abstract class BasePersistableEntity implements InstanceCallbacks, Serializable {
    ...
    @PropertyLayout(hidden = Where.ALL_TABLES, named = "Created On", renderedAsDayBefore = true)
    @Getter @Setter
	...
	protected void initializeForCreate(BasePersistableEntity bean) {
		bean.setDateCreation(new Date());
		if(bean.getUserCreation()==null) {
			bean.setUserCreation("system"); // TODO
		}
		bean.setProgramCreation("system"); // TODO
	}

	protected void initializeForModification(BasePersistableEntity bean) {
	...
	public void jdoPreStore() {
		if(JDOHelper.isNew(this)) {
    		initializeForCreate(this);
		} else {
			initializeForModification(this);
		}
	}
